...
public class ZbChargerController : MonoBehaviour
{
    GameManager GM;
    //NavMesh Needed
    NavMeshAgent agent;
   ...
   // public GameObject gp;
    ...
    public void Awake()
    {
        ...
        // PlayerController plr= GM.player.GetComponent<PlayerController>();
    ...
    void Update()
    {
        ...
        if (rdsound < 0)
        {
            FindObjectOfType<ZbCAudio>().Play("ZbIdle");
        }
        if (!PlayerDead)
        {
    ...
    {
        // PlayerController gps = GM.player.GetComponent<PlayerController>();
        PlayerController gps = GM.playerController;
      
        if (GM.playerHP > 0)
        // if (gps.hp > 0)
        {
            FindObjectOfType<ZbCAudio>().Play("ZbHit");
            gps.TakingDmg = true;
            // gps.hp -= 40;
            GM.reducePlayerHP(0.4f);
        }
        else
        {
            PlayerDead = true;
    ...
    {
        FindObjectOfType<ZbCAudio>().Play("ZbScream");
    ...
    public void ViewBlocked()
    {
        ...
        viewblocked = Physics.Raycast(this.transform.position,offset,dist-3.0f);
        
    }
    public void gotHit(){
        //zombie hp
        // Debug.Log("Hit boss zombie");
